...
export function registerGetMainClient(server: McpServer, client: TravelCodeApiClient) {
  server.tool(
    ...
    [
      ...
      "",
      "Impersonation — READ FIRST:",
      "  • This endpoint returns the profile of whoever is on the request. Without actAs you get the ADMIN's traveler, not the target user's.",
      "  • If the upcoming search/booking will run with actAs=<email> (admin acting for another user), you MUST call this tool with the SAME actAs (and actAsCompanyId when set). Otherwise the nationality you read here belongs to the admin and the downstream search will be priced for the wrong user.",
      "  • Do NOT pre-fetch this tool with no actAs and then reuse the result later for an impersonated call — that is the most common mistake. If actAs becomes known after a no-actAs call, refetch with the correct actAs and discard the previous result.",
      "",
      "When to call:",
